...
import { Users, Activity, AlertTriangle, Settings, FileSearch, Shield, FileText } from "lucide-react";
...
const TABS = [
  ...
  { to: "/app/admin/templates", label: "Private Templates", icon: FileText },
...
function AdminLayout() {
  ...
  useEffect(() => {
    if (!loading && user && user.role !== "admin") navigate({ to: "/app" });
  ...
  return (
    <div>
      <div className="px-4 sm:px-8 md:px-10 pt-6 pb-4 border-b border-border">
          ...
          {TABS.map((t) => {
            ...
            return (
              <Link key={t.to} to={t.to}
                className={`inline-flex items-center gap-2 px-3 sm:px-4 py-2 rounded-xl text-xs sm:text-sm font-medium transition-colors whitespace-nowrap ${
