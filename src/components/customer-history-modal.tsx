...
function TransmissionPanel({ userId }: { userId: string }) {
  const [rows, setRows] = useState<Awaited<ReturnType<typeof api.adminCustomerTransmission>> | null>(null);
  const [filter, setFilter] = useState("all");
  const [search, setSearch] = useState("");
  useEffect(() => { api.adminCustomerTransmission(userId).then(setRows).catch(() => {}); }, [userId]);
  const filtered = (rows || []).filter((r) => {
    if (filter !== "all" && (r.status || "").toLowerCase() !== filter) return false;
    if (search && !`${r.email} ${r.subject}`.toLowerCase().includes(search.toLowerCase())) return false;
    return true;
  });
  ...
  return (
    <div className="p-5 space-y-3">
      <div className="flex flex-wrap items-center gap-2">
        <input placeholder="Search email / subject…" value={search} onChange={(e) => setSearch(e.target.value)}
          className="flex-1 min-w-[180px] px-3 py-2 text-xs rounded-md bg-card/50 border border-border focus:outline-none" />
        <div className="flex flex-wrap gap-1 text-xs">
          {STATUSES.map((s) => (
            <button key={s} onClick={() => setFilter(s)}
              className={`px-2 py-1 rounded-md capitalize ${filter === s ? "bg-primary/20 text-primary-foreground border border-primary/40" : "text-muted-foreground hover:text-foreground"}`}>{s}</button>
          ))}
        </div>
        <button onClick={() => api.exportTransmissionCsv({ userId })} className="text-xs inline-flex items-center gap-1 px-3 py-1.5 rounded-md glass hover:bg-card/60">
          <Download size={12} /> CSV
        </button>
      </div>
      <div className="overflow-x-auto rounded-lg border border-border max-h-[55vh] overflow-y-auto">
        <table className="w-full text-xs">
          ...
          <tbody>
            {rows === null ? <tr><td colSpan={6} className="p-6 text-center text-muted-foreground">Loading…</td></tr>
            : filtered.length === 0 ? <tr><td colSpan={6} className="p-6 text-center text-muted-foreground">No events.</td></tr>
            : filtered.map((r, i) => (
              <tr key={i} className="border-t border-border/40 hover:bg-card/30">
                <td className="px-3 py-2 whitespace-nowrap text-muted-foreground">{r.lastEventAt ? new Date(r.lastEventAt).toLocaleString() : new Date(r.createdAt).toLocaleString()}</td>
                <td className="px-3 py-2 font-mono">{r.email}</td>
                <td className="px-3 py-2 truncate max-w-[220px]">{r.subject}</td>
                <td className="px-3 py-2 uppercase tracking-wider">{r.status}</td>
                <td className="px-3 py-2 text-muted-foreground">{r.eventType || "—"}</td>
                <td className="px-3 py-2 text-destructive truncate max-w-[200px]">{r.reason || ""}</td>
              </tr>
            ))}
          </tbody>
        </table>
